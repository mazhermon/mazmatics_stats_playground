...
import { getDb, type EndorsementRow } from '@/lib/db';
...
export async function GET(request: NextRequest) {
  ...
  const queryStr = `SELECT * FROM qualification_endorsement ${where} ORDER BY year, year_level, qualification`;

  try {
    const sql = getDb();
    const rows = await sql.unsafe(queryStr, params) as EndorsementRow[];
    return NextResponse.json({ data: rows, count: rows.length });
  } catch (error) {
    console.error('[/api/nzqa/endorsements]', error);

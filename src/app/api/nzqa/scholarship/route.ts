...
import { getDb, type ScholarshipRow } from '@/lib/db';
...
export function GET(request: NextRequest) {
  ...
  const sql = `SELECT * FROM scholarship ${where} ORDER BY year, subject`;

  try {
    const db = getDb();
    const rows = db.prepare(sql).all(params) as ScholarshipRow[];

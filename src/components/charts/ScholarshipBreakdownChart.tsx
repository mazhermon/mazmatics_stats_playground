...
export function ScholarshipBreakdownChart() {
  ...
  useEffect(() => {
    ...
    setError(null);
    fetch(
      `/api/nzqa/scholarship?subject=${encodeURIComponent(subject)}&groupBy=${groupBy}&yearFrom=${effectiveYear}&yearTo=${effectiveYear}`
    )
      .then((r) => r.json())
  ...
  useEffect(() => {
    ...
    const bandNames: Array<'no_award' | 'scholarship' | 'outstanding'> = ['no_award', 'scholarship', 'outstanding'];

    // Sort by award rate descending
    const sorted = [...data].sort(
      (a, b) =>
        ((b.outstanding_rate ?? 0) + (b.scholarship_rate ?? 0)) -
        ((a.outstanding_rate ?? 0) + (a.scholarship_rate ?? 0))
    );
    ...
    sorted.forEach((row, i) => {
      ...
      // Stacked bars: no_award first (left), then scholarship, then outstanding
      let xOffset = 0;
      bandNames.forEach((band) => {
        ...
        if (bw > 34) {
          g.append('text')
            ...
            .attr('fill', band === 'outstanding' ? '#1e293b' : '#e2e8f0')
            .attr('font-size', '10px')
            .attr('font-weight', '600')
            .text(fmtRate(rate));
        }

        const bandLabel = band === 'no_award' ? 'No Award' : band.charAt(0).toUpperCase() + band.slice(1);
        rect
          .on('mousemove', (event: MouseEvent) => {
            ...
            setTooltip({
              ...
              html: `<strong>${label}</strong><br/>${bandLabel}: <strong>${fmtRate(rate)}</strong>${row.total_assessed ? `<br/>Assessed: ${row.total_assessed.toLocaleString('en-NZ')}` : ''}`,
              visible: true,
            });
          })
      ...
      // n= label
      if (row.total_assessed) {
        g.append('text')
          .attr('x', xOffset + 6)
          .attr('y', y + bh / 2 + 4)
          .attr('fill', '#475569')
          .attr('font-size', '10px')
          .text(`n=${row.total_assessed.toLocaleString('en-NZ')}`);
      }
    });
